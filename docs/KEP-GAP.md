...
| **Member credentials** | ClusterProfile `status.accessProviders` plugin mechanism (KEP-4322/5339) | **Simplified** (e.g. kubeconfig-based access provider) | ClusterProfile is still the inventory API; only the credential resolution is simplified for kind. |
