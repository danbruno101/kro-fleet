...
| **Empty placement** | Terminal, explicit `Placed=False` with a machine-readable reason; never a fallback to a broader set | **Not distinguished** from "not ready yet" — worse, `RollupReady` reports `Ready=True / MinReadyClustersMet` on **zero** placed clusters when `tolerance.minReadyClusters: 0` is set explicitly (`internal/controller/placement.go`) | Required for the compliance use case: there is no graceful fallback for a hard constraint. The explicit-zero edge must become a refusal, not a vacuous success. |
